...
for yr in years:
      ...
      for qtr in quarters:
        ...
        # get the absolute file path to the empty text file(on which master.zip content to be downloaded)we created locally on our pc   
           absolute_file_path='/'.join([master_path, str(yr),base_file_path])
           
        # Define the url of the master.zip file to be downloaded
           url = f'https://www.sec.gov/Archives/edgar/full-index/{yr}/{qtr}/master.zip'
         ...
         'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/80.0.3987.163 Safari/537.36',
          ...
          #create the  folder before creating file in it 
           ...
           #extract the index.zip content file as master_files/{yr}/master.idx
           z=ZipFile(io.BytesIO(response.content))  
           z.extractall('/'.join([master_path, str(yr)]))  
           
           #rename master.idx to master-index-{yr}-{qtr}.txt
           os.rename('/'.join([master_path, str(yr),"master.idx"]),absolute_file_path)
      
        # Get the corrosponding master.idx file from the website and save it to a corrosponding empty text file.
           f=open(absolute_file_path, 'rb')
